...
st = 'workout' + st + '.csv'
file = open(st, 'w')
file.write('Time, Distance, Heartrate, power, SPM, Pace, Force Plot\n')
...
while workout['state'] == 1:
	...
	#Write data to file
	workoutdata = str(monitor['time']) + "," + str(monitor['distance']) + "," + str(monitor['heartrate']) + "," + str(monitor['power']) + "," + str(monitor['spm']) + "," + str(monitor['pace']) + ","
